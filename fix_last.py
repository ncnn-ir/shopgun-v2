...
# ═══════════════════════════════════════════════════════════════
# 1. حذف دکمه جدول حرفه‌ای از certificates index
# ═══════════════════════════════════════════════════════════════
...
if idx.exists():
    ...
    # حذف دکمه جدول حرفه‌ای
    txt = re.sub(
        r'<a href="\{\{ route\(\'certificates\.table\'\) \}\}"[^<]*</a>\s*',
        '',
        txt
    )
    txt = re.sub(
        r'<a[^>]*certificates\.table[^>]*>.*?</a>\s*',
        '',
        txt,
        flags=re.DOTALL
    )
    ...
    print("[OK] certificates index - دکمه جدول حرفه‌ای حذف شد")


# ═══════════════════════════════════════════════════════════════
# 2. حذف route و فایل‌های جدول حرفه‌ای
# ═══════════════════════════════════════════════════════════════
...
if routes.exists():
    ...
    txt = re.sub(
        r"\s*Route::get\('/table',[^;]*;\s*",
        '\n        ',
        txt
    )
    routes.write_text(txt, encoding='utf-8')
    print("[OK] routes - حذف /certificates/table")


# ═══════════════════════════════════════════════════════════════
# 3. CREATE.PHP — بهبود تشخیص ابعاد + معادل‌یابی سنگ
# ═══════════════════════════════════════════════════════════════
